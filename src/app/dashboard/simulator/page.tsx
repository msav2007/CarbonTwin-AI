  ...
  if (!hasHydrated || !result) {
    return (
      <Container className="py-20 text-center">
        <p className="text-[#94A3B8]">Loading simulator...</p>
      </Container>
    );
  }
      ...
      <main className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <Card className="h-full border-cyan-500/[0.1] bg-gradient-to-b from-cyan-500/[0.02] to-transparent">
              <CardHeader>
                <div className="flex items-center gap-2">
                  <ShieldCheck className="h-5 w-5 text-primary" />
                  <CardTitle>Steady Path</CardTitle>
                </div>
                <p className="text-sm text-[#94A3B8]">{result.simulations.steady.summary}</p>
              </CardHeader>
              <CardContent>
            ...
            <Card className="h-full border-amber-500/[0.1] bg-gradient-to-b from-amber-500/[0.02] to-transparent">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-amber-400">Ambitious Path</CardTitle>
                </div>
                <p className="text-sm text-[#94A3B8]">{result.simulations.ambitious.summary}</p>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {result.simulations.ambitious.outlook.map((outlook) => (
                    <div key={outlook.years} className="rounded-xl border border-white/5 bg-black/20 p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-mono text-sm font-bold text-[#F8FAFC]">Year {outlook.years}</span>
                        <Badge variant="outline" className="text-xs text-amber-300">Score: {outlook.scoreIfMaintained}</Badge>
                      </div>
                      <div className="flex justify-between items-end mb-3">
                        <div>
                          <p className="text-[10px] uppercase text-[#94A3B8]">Baseline</p>
                        ...
                        <div className="text-right">
                          <p className="text-[10px] uppercase text-[#94A3B8]">Projected</p>
                          <p className="text-2xl font-bold text-amber-400">{formatTonnes(outlook.plannedAnnualKg)}t</p>
                        </div>
                      </div>
                      <p className="text-xs text-[#94A3B8]">{outlook.narrative}</p>
                    </div>
