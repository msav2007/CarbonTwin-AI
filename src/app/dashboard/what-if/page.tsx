  ...
  if (!hasHydrated || !originalData || !originalResult || !testData) {
    return (
      <Container className="py-20 text-center">
        <p className="text-[#94A3B8]">Loading Time Machine...</p>
      </Container>
    );
  }
          ...
          <div className="lg:col-span-7 space-y-6">
            <Card className="border-cyan-500/[0.08] bg-white/[0.02]">
              ...
              <CardContent className="space-y-6">
                <div>
                  <label htmlFor="transport-select" className="text-sm font-medium text-[#F8FAFC] mb-2 block">Transport</label>
                  <select 
                    id="transport-select"
                    className="w-full bg-[#08111B] border border-cyan-500/20 rounded-lg p-2 text-sm text-[#94A3B8]"
                ...
                <div>
                  <label htmlFor="diet-select" className="text-sm font-medium text-[#F8FAFC] mb-2 block">Diet</label>
                  <select 
                    id="diet-select"
                    className="w-full bg-[#08111B] border border-cyan-500/20 rounded-lg p-2 text-sm text-[#94A3B8]"
                    value={testData.diet}
                ...
                <div>
                  <label htmlFor="energy-select" className="text-sm font-medium text-[#F8FAFC] mb-2 block">Home Energy</label>
                  <select 
                    id="energy-select"
                    className="w-full bg-[#08111B] border border-cyan-500/20 rounded-lg p-2 text-sm text-[#94A3B8]"
                ...
                <div>
                  <label htmlFor="travel-select" className="text-sm font-medium text-[#F8FAFC] mb-2 block">Travel Flights</label>
                  <select 
                    id="travel-select"
                    className="w-full bg-[#08111B] border border-cyan-500/20 rounded-lg p-2 text-sm text-[#94A3B8]"
                ...
                <div>
                  <label htmlFor="shopping-select" className="text-sm font-medium text-[#F8FAFC] mb-2 block">Shopping Habits</label>
                  <select 
                    id="shopping-select"
                    className="w-full bg-[#08111B] border border-cyan-500/20 rounded-lg p-2 text-sm text-[#94A3B8]"
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-6">
                    <div className="text-center flex-1">
                      <p className="text-xs uppercase text-[#94A3B8]">Current</p>
                      <p className="font-display text-2xl font-bold text-[#F8FAFC]">{formatTonnes(originalResult.annualKg)}t</p>
                    </div>
                    <ArrowRight className="h-5 w-5 text-[#94A3B8]/50" />
                    <div className="text-center flex-1">
                      <p className="text-xs uppercase text-[#94A3B8]">Scenario</p>
                  ...
                  <div className={`rounded-xl p-4 text-center ${isBetter ? 'bg-success/10 border border-success/20' : diffKg === 0 ? 'bg-white/5 border border-white/10' : 'bg-red-500/10 border border-red-500/20'}`}>
                    <p className="text-sm text-[#94A3B8] mb-1">Difference</p>
                    <p className={`font-display text-3xl font-bold ${isBetter ? 'text-success' : diffKg === 0 ? 'text-[#F8FAFC]' : 'text-red-400'}`}>
                      {isBetter ? '-' : diffKg === 0 ? '' : '+'}{Math.abs(diffKg)} kg/yr
                    </p>
                  </div>

                  <div className="mt-6">
                    <p className="text-sm font-medium text-[#F8FAFC] mb-2">New Score Prediction</p>
                    <div className="flex justify-between items-center bg-black/20 rounded-lg p-3 border border-white/5">
                      <span className="text-[#94A3B8]">Carbon Score</span>
